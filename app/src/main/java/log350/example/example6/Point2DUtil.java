...
public class Point2DUtil {
	...
	// Use 1 finger to translate shape
	static public void translatePointsBasedOnDisplacementOfOnePoint(ArrayList<Point2D> points, Point2D P_old, Point2D P_new) {
		Point2D centroid = computeCentroidOfPoints( points );
		Vector2D v1 = Point2D.diff( P_old, centroid );
		Vector2D v2 = Point2D.diff( P_new, centroid );
		float lengthToPreserve = v1.length();
		...
		for ( Point2D p : points ) {
			p.get()[0] = p.get()[0] + translation.x();
			p.get()[1] = p.get()[1] + translation.y();
		}
	}
