...
public class DrawingView extends View {
	...
	ArrayList<Point2D> positionsDoigts= new ArrayList<Point2D>();
	...
	static final int MODE_ZOOM = 4; // the user is drawing a lasso to select shape
	static final int MODE_EFFACER = 5; // the user is erasing the selected shapes
  static final int MODE_CREER = 6; // the user is creating shapes
	...
	MyButton zoomButton = new MyButton( "Zoom", 10, 230, 140, 140 );
  MyButton effacerButton = new MyButton( "Effacer", 10, 390, 140, 140 );
	MyButton creerButton = new MyButton( "Créer", 10, 550, 140, 140 );
				...
				public boolean onTouch(View v, MotionEvent event) {
					...
					case MODE_CAMERA_MANIPULATION :
						...
						if ( cursorContainer.getNumCursors() == 1 && type == MotionEvent.ACTION_MOVE ) {
							MyCursor cursor0 = cursorContainer.getCursorByIndex( 0 );
							gw.moveCamera(cursor0.getPreviousPosition(),cursor0.getCurrentPosition());
						}
					...
					case MODE_SHAPE_MANIPULATION :
                        ...
                        if( cursorContainer.getNumCursors()==1 && type == MotionEvent.ACTION_MOVE && !(selectedShapes.isEmpty()) &&  selectedShapes.contains(shapeContainer.getShape( indexOfShapeBeingManipulated ))){
                            MyCursor cursor0 = cursorContainer.getCursorByIndex( 0 );
						...
						case MODE_CREER :
							if ( type == MotionEvent.ACTION_DOWN ) {
								for(int i=1;i<cursorContainer.getNumCursors();i++) {
									positionsDoigts.add(gw.convertPixelsToWorldSpaceUnits(cursorContainer.getCursorByIndex(i).getCurrentPosition()));

								}
								positionsDoigts = Point2DUtil.computeConvexHull(positionsDoigts);

							}
							else if ( type == MotionEvent.ACTION_UP ) {
								shapeContainer.addShape(positionsDoigts);
								positionsDoigts.clear();
								cursorContainer.removeCursorByIndex( cursorIndex );
								if ( cursorContainer.getNumCursors() == 0 ) {
									currentMode = MODE_NEUTRAL;
								}
